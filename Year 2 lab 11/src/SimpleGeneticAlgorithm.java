...
public class SimpleGeneticAlgorithm {
	...
	// Run the Genetic Algorithm with the current parameter set, except for
	// generations
	...
	public double ComputeAverage() {
		...
		for (int i = 0; i < population.size(); ++i)
			av += population.get(i).GetFitness();
	...
	private void CrossOver() {
		...
		while (xlist.size() > 1) {
			...
			// Comment out the above and uncomment the following for Uniform
			// Crossover
	...
	private void Mutation() {
		...
		for (int i = 0; i < n; ++i) {
			...
			if (changed)
				population.add(new ScalesChrome(chrome));
		}
	}
